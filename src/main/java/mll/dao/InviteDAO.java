    ...
    public Invite generateInvite(Invite invite) throws Exception {
        ...
        try {
            ...
            if (null != invite && null != invite.getToken().getToken()) {
                // This increments the tokenId key in the database and assigns it to the new token
                Serializable tokenId = session.save(invite.getToken());
                invite.getToken().setId((Integer) tokenId);
